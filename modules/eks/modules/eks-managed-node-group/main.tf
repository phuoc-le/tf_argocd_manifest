...
locals {
  launch_template_name = coalesce(var.launch_template_name, "${var.cluster_name}-node-group")
...
resource "aws_launch_template" "this" {
  ...
  dynamic "tag_specifications" {
    ...
    content {
      ...
      tags          = merge(var.tags, { Name = "${var.cluster_name}-${var.name}" }, var.launch_template_tags)
    }
  }
...
locals {
  iam_role_name          = coalesce(var.iam_role_name, "${lower(var.env)}-${var.name}-eks-node-group")
